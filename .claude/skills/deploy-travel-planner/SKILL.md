...
2. **Confirm `.env` exists and has a real `GEMINI_API_KEY`.**
   ```bash
   test -s .env && grep -E "^GEMINI_API_KEY=.+" .env || echo "MISSING"
   ```
...
5. **Smoke test.**
   ...
   - Log in, open a trip, confirm the AI chat responds (proves `GEMINI_API_KEY` was inlined correctly).
...
- **Blank white page** → `GEMINI_API_KEY` was missing at build time. Rebuild with `.env` present and redeploy.
